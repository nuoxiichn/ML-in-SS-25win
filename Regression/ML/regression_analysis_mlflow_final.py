...
def main():
    ...
    with mlflow.start_run(run_name="Airfoil_Pipeline") as parent_run:
        ...
        X = data_filt.drop(columns=[target_vble,'attack-angle']) # Eliminate one feature with high correlation with target to see if it improves generalization
        ...
        param_grid = {
            'n_estimators': [200, 500, 1000],          # 增加树的数量通常能更稳
            'max_depth': [None, 20, 30, 40, 50],               # 包含 None，允许深度生长
            'min_samples_split': [2, 5],               # 包含默认值 2
            'min_samples_leaf': [1, 2],                # 包含默认值 1
            'max_features': [None, 'sqrt'],            # None 表示使用全部特征
        }
        
        with mlflow.start_run(run_name="Tuning - RF RandomSearch", nested=True):
            # Save and log parameter grid
            param_grid_path = os.path.join(PATH_CSV, "rf_param_space.json")
            ...
            random_search = RandomizedSearchCV(
                ...
                n_jobs=1, # storage and CPU constraints!
                verbose=1,
                random_state=42
            )
            ...
            cv_results_path = os.path.join(PATH_CSV, "rf_random_search_cv_results.csv")
        ...
        final_params = {
            ...
            'model_type': 'RandomForestRegressor_Final'
        }
        ...
        print(f'RMSE (dB): {final_metrics["test_rmse"]:.4f}')
        print(f'MAE (dB): {final_metrics["test_mae"]:.4f}')
        print(f'MAPE (%): {final_metrics["test_mape"]*100:.4f}')
        print(f'R2 (-): {final_metrics["test_r2"]:.4f}')
        print(f'R2 Adjusted (-): {final_metrics["test_r2_adjusted"]:.4f}')
